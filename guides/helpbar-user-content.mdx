In this guide we will talk about how to make your User-generated content ([SearchItem](/api/search#schema-search-items)s) searchable with the HelpBar. This guide makes some assumptions
...
- Pull the `SEARCH_GROUP_ID` from the URL for the Chameleon [SearchGroup](/api/search#schema-search-groups) that you want to add this content to (e.g. `https://app.chameleon.io/helpbar/edit/content/654001081b27ac5e6ae5a08e`)
...
- Use 1 [SearchActions](/api/search#schema-search-actions) in `actions` with `kind=navigate`. When clicked in the HelpBar
