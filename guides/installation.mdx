...
Chameleon is designed to mirror this in its design of [User Profiles](/api/profiles), [Companies](/api/companies),
[Segments](/api/segments), searchable [HelpBar content](/guides/helpbar/user-generated-content), etc.
...
Imagine that you want to send your Users to the billing page. With this script you can setup a Navigate type of HelpBar [SearchItem](/api/helpbar#schema-search-items) for the Billing page and link to `/settings/billing`.
When this item is clicked you will receive a callback to `app:navigate` with opts set to `{ to: "/settings/billing" }`

#### React-app-navigate-js
