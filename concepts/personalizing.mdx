...
|----------------------------|----------------------------------------------------------------|---------------------------------------------------------------------------------------|
| `property`                 | [examples ↓](#examples-property)   | The default used when a unquoted string is found at the beginning                     |
| `global`                   | [examples ↓](#examples-global)     | Pull a value from the window object, useful for extra-advanced conditional formatting |
| `pluralize`                | [examples ↓](#examples-plural)     | Given a specific number and a word produces a phrase with the correct tense           |
| `time_difference_in_words` | [examples ↓](#examples-time-diff)  | Given a specific date/time produces a time offset                                     |
| `time_ago`                 | [examples ↓](#examples-time-ago)   | Calculates the offset from now in `seconds`,`minutes`,`hours`,`days`,`weeks`,`years`  |
| `time_local`               | [examples ↓](#examples-time-local) | Given a specific date/time uses toLocalString() to generate a human readable string   |
| `delivery`                 | [examples ↓](#examples-delivery)   | Personalize with content explicitly sent via a [Delivery](/api/deliveries)         |
| `html`                     | [examples ↓](#examples-html)       | Output html based on given options                                                    |
| `filter`                   | [examples ↓](#examples-filter)     | Use filter to evaluate [Segmentation filter expressions](/concepts/filters)         |                                  |
| `if` (content logic)       | [examples ↓](#examples-logic-if)   | Output html based on given options                                                    |
...
This calculation is best used as a nested helper in an `if` block | [examples ↓](#examples-logic-if)
...
**As part of an `if` block** | [more examples ↓](#examples-logic-if)
