...
Your secret token can be [generated here](https://app.chameleon.io/settings/tokens). All tokens are account-specific and can only be downloaded once.

<Warning>
Make sure you keep your secret token secure. Do not share it in public accessible areas, as it represents the right to access your data.
</Warning>

To check your Account Secret token, make one of these requests:
...
```bash POST Body
curl -X POST -d '{"account_secret":"ACCOUNT_SECRET"}' \
  -H "Content-Type: application/json" \
  https://api.chameleon.io
```
</CodeGroup>
...
Chameleon uses a defined set of IP addresses to send webhooks and any other traffic **from** Chameleon. Consider the list below to be static; any updates will be posted to our [Status Page](https://status.chameleon.io).
...
<Note>
Traffic into Chameleon's services will not adhere to this list and will mostly resolve to [Fastly](https://www.fastly.com/).
</Note>