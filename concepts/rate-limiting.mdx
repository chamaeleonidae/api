...
- [User Profile Searching](/api/profiles-search) with `max_concurrent=1` or in situations where the API is being used abusively.
- [Creating a Delivery](/api/deliveries) with `max_concurrent=1` per User Profile to handle the enforcement of the maximum [Pending Deliveries](/api/deliveries#limits).
