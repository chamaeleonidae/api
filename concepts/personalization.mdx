Personalization is the way to adjust Experience content to match the User who is viewing an Chameleon Experience.

---

| Helper name                | -                                                              | description                                                                           |
|----------------------------|----------------------------------------------------------------|---------------------------------------------------------------------------------------|
| `property`                 | [examples ↓](#examples)   | The default used when a unquoted string is found at the beginning                     |
| `global`                   | [examples ↓](#examples-for-global-helper)     | Pull a value from the window object, useful for extra-advanced conditional formatting |
| `pluralize`                | [examples ↓](#examples)     | Given a specific number and a word produces a phrase with the correct tense           |
| `time_difference_in_words` | [examples ↓](#examples)  | Given a specific date/time produces a time offset                                     |
| `time_ago`                 | [examples ↓](#examples)   | Calculates the offset from now in `seconds`,`minutes`,`hours`,`days`,`weeks`,`years`  |
| `time_local`               | [examples ↓](#examples) | Given a specific date/time uses toLocalString() to generate a human readable string   |
| `delivery`                 | [examples ↓](#examples-using-delivery-helper)   | Personalize with content explicitly sent via a [Delivery](/api/deliveries)         |
| `html`                     | [examples ↓](#examples)       | Output html based on given options                                                    |
| `filter`                   | [examples ↓](#examples)     | Use filter to evaluate [Segmentation filter expressions](/concepts/filters)         |                                  |
| `if` (content logic)       | [examples ↓](#examples)   | Output html based on given options                                                    |
...
Use the `property` helper to insert user data into your content.

**Example:** Display user's first name with fallback option.
...
This calculation is best used as a nested helper in an `if` block | [examples ↓](#use-custom-logic--if-block-helper)
...
**As part of an `if` block** | [more examples ↓](#use-custom-logic--if-block-helper)
...
<Tip>Use for feature launches, maintenance windows, expiration dates for surveys/deals, etc.</Tip>
...
<Note>This function should be added to your product's codebase to provide extended functionality. In this example we will assume you have a `performAction` function defined like this:</Note>
...
<Warning>When passing user properties to functions, use the property name **without quotes or double braces**. Quoted values (`'text'`) are passed as literal strings, while unquoted properties (`user_property`) are resolved to the user's actual property value.</Warning>
...
<Note>The `delivery` helper documented here only works for the specific Experience that was triggered via Delivery.</Note>
...
Now, to use the merge tag (based on the [example data ↑](#example-user-data))
