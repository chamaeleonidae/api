...
- Getting access to the A/B testing attribute (`percent`) when the Chameleon [User Profile](/api/profiles#schema) is loaded.
- Logging Experience data to an integration that Chameleon does not yet have a native integration with
- Knowing when the [HelpBar](/api/helpbar#search-javascript-api) is opened/closed or when data is needed.
...
<Info>**Typical ordering as Chameleon boots**: `load`, `load:chmln`, `after:account`, `identify:request`, `identify:sync`, `after:profile`, `tour:event`, `chmln:event`</Info>

## Examples

#### HelpBar

The [JavaScript API for HelpBar](/api/helpbar#search-javascript-api) has many examples and use cases.
