...
It is typically delivered as an embed on your marketing site, or as part of a Chameleon [Tour](apis/tours).
...
<Note>The demo can run in Profile or Anonymous modes as outlined below. If possible, run Product Demos in Profile mode</Note>

## Embedding

<Note>Replace the `:id` below with the Demo ID from the dashboard OR copy the embed code from the Sharing section.</Note>

### Examples {#examples}

#### Profile mode (logged-in user ID) {#mode-profile}

Any and all times that you have information about the logged-in user. All of the information about what properties/data
to send is in the main documentation about [Identifying users](js/profiles) but the Tl;DR is that you should send at least a
...
<Accordion title="Adding User data">
...
For simplicity, and unless otherwise specified, adding User ID for an identified user, changes to [consent](js/demos#cookie-consent) of `granted`.
...
#### Anonymous mode (the default) {#mode-anonymous}
...
### Consent to track (cookies etc.) {#cookie-consent}
...
<Note>Typically, Product Demos fall into the **Functional** category.</Note>
...
engagement with Product Demos into the other experiences that Chameleon offers such as [Tours](apis/tours)),
[Microsurveys](apis/surveys), and [HelpBar](apis/search).
...
### Message event kinds {#schema-kinds}
...
#### Generic analytics tracking {#example0}
...
#### Custom handling of individual events {#example2}
...
## Speed, Performance, Caching {#caching}
