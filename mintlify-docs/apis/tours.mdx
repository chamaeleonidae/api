---
title: "Tours"
description: "A Tour is a sequence of steps that shows to your end-users when they meet all of the predefined matching criteria, including:"
---
...
`scheduled_publish_at` and `scheduled_unpublish_at` are **read-only**. They report a schedule set elsewhere in Chameleon, and cannot be set or cleared by [updating a Tour](#campaigns-update).

Both report a **pending** schedule only, never history. Chameleon clears the timestamp the moment the schedule runs, so a Tour that has already gone dark on schedule returns `scheduled_unpublish_at: null` rather than the time it stopped. Read `published_at` to tell whether a Tour is currently live — it becomes `null` once the Tour is unpublished.

A pending schedule can also disappear **without** the Tour publishing or unpublishing: if the person who set it loses the *Publish Experiences* permission, or leaves the team, before the schedule runs, Chameleon cancels the schedule and emails everyone on your team who can publish Experiences instead. The Tour is left exactly as it was, and the timestamp becomes `null`.
