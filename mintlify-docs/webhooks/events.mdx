...
A 200 is not always a terminal result. Where the body reports `errors`, correct those items before sending them again — retrying them unchanged fails the same way. Where it does not, the same batch can succeed on a later retry, once the User Profiles it addresses exist.
