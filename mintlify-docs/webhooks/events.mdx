...
With `on_model_missing=ignore` an item whose User Profile is not found is skipped without an `errors` entry, so a batch in which no User Profile is found returns a 200 with `"count": 0` and **no `errors` key at all**.
...
A 200 is not always a terminal result. Whether an item can be sent again unchanged depends on the error it carried.
...
- A 200 that carries no `errors` at all is the same lookup miss, with the per-item reporting suppressed by `on_model_missing=ignore`. The same batch can succeed on a later retry, once the User Profiles it addresses exist.
