...
description: "Receive real-time events when users interact with your Chameleon Experiences. Configure endpoints to receive Tour, Survey, Embed, Demo, HelpBar, and Alert webhooks."
---

<a id="outgoing-webhooks"></a>
## Data out of Chameleon
A webhook is an agreed-upon method of data exchange across a **secure channel**. Since you will be adding a new endpoint to your backend servers to handle this webhook, is it **strongly recommended** that you [verify the signature](#verification) of any webhook requests before processing any included data.
...
When receiving a webhook from Chameleon you should:
 ...
 - [Verify](#verification) the Webhook request signature; respond with a status `400` if the signature does not match
...
|-------------------------|----------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `ping`                  | [example](#example-ping)                | Account                                                                                                                                            | Sent as a simple check to make sure the endpoint is working                                                                                                                                                                                       |
| `response.finished`     | [example](#example-response-finished)   | [Response](/apis/survey-responses), [Microsurvey](/apis/surveys), [User Profile](/apis/profiles)                                             | Sent when the Microsurvey is finished (all Steps completed; including text comment if configured)                                                                                                                                                 |
| `alert.triggered`       | [example](#example-alert-triggered)     | [Alert Group](/apis/alert-groups), [Experiences](/concepts/experiences)                                                                        | Sent when an Alert is triggered by a violation of the alerting conditions                                                                                                                                                                         |
| `helpbar.answer`        | [example](#example-helpbar-answer)      | [Action](#schema-actions-helpbar-answer), [User Profile](/apis/profiles)                                                          | Sent when a Search query generates an AI Answer in HelpBar                                                                                                                                                                                        |
| `helpbar.search`        | [example](#example-helpbar-search)      | [Action](#schema-actions-helpbar-search), [User Profile](/apis/profiles)                                                          | Sent when a Search query displays its results in the HelpBar                                                                                                                                                                                      |
| `helpbar.item.action`   | [example](#example-helpbar-item-action) | [Action](#schema-actions-helpbar-item-action), [User Profile](/apis/profiles)                                                     | Sent when an item in the HelpBar is clicked/actioned                                                                                                                                                                                              |
| `helpbar.item.error`    | [example](#example-helpbar-item-error)  | [Action](#schema-actions-helpbar-item-error), [User Profile](/apis/profiles)                                                      | Sent when an action from an item in the HelpBar throws an error when running                                                                                                                                                                      |
| `tour.started`          | [example](#example-tour-all)            | [Action](#schema-actions-campaign), [Tour](/apis/tours), [Step](/apis/steps), [User Profile](/apis/profiles)          | Sent when the Tour is started; includes the first Step in the payload                                                                                                                                                                             |
| `tour.completed`        | [example](#example-tour-all)            | [Action](#schema-actions-campaign), [Tour](/apis/tours), [Step](/apis/steps), [User Profile](/apis/profiles)          | Sent when the Tour is completed; includes the Step the user completed                                                                                                                                                                             |
| `tour.exited`           | [example](#example-tour-all)            | [Action](#schema-actions-campaign), [Tour](/apis/tours), [Step](/apis/steps), [User Profile](/apis/profiles)          | Sent when the Tour is exited; includes the Step the user exited                                                                                                                                                                                   |
| `tour.snoozed`          | [example](#example-tour-snooze)         | [Action](#schema-actions-snooze), [Tour](/apis/tours), [Step](/apis/steps), [User Profile](/apis/profiles)            | Sent when the Tour is exited on Step configured to snooze (re-display the Step at a later time).                                                                                                                                                  |
| `tour.button.clicked`   | [example](#example-tour-button-clicked) | [Tour](/apis/tours), [Step](/apis/steps), [Button](/apis/buttons), [User Profile](/apis/profiles)                                          | Sent when the Tour is exited with the Step the user exited                                                                                                                                                                                        |
| `survey.started`        | [example](#example-survey-all)          | [Action](#schema-actions-campaign), [Microsurvey](/apis/surveys), [Step](/apis/steps), [User Profile](/apis/profiles) | Sent when the Microsurvey is started with the first Step                                                                                                                                                                                          |
...
| `embed.snoozed`         | [example](#example-embed-snooze)        | [Action](#schema-actions-snooze), [Embed](/apis/tours), [Step](/apis/steps), [User Profile](/apis/profiles)           | Sent when the Embed is exited on Step configured to snooze (re-display the Step at a later time).                                                                                                                                                 |
| `embed.button.clicked`  | [example](#example-tour-button-clicked) | [Embed](/apis/tours), [Step](/apis/steps), [Button](/apis/buttons), [User Profile](/apis/profiles)                                         | Sent when the Embed is exited with the Step the user exited                                                                                                                                                                                       |
| `demo.started`          | [example](#example-demo-started)        | [Demo](/apis/demos), [DemoRun](/apis/demos#schema-demo-run), [User Profile](/apis/profiles)                                               | Sent when the Demo is started                                                                                                                                                                                                                     |
| `demo.reveal`           | [example](#example-demo-reveal)         | [Demo](/apis/demos), [DemoRun](/apis/demos#schema-demo-run), [User Profile](/apis/profiles)                                               | Sent after the Demo is started _and_ [Clearbit reveal](https://app.chameleon.io/integrations/clearbit) is enabled _and_ a match is found. The `demo_run` will have `reveal_domain` and `reveal_name` keys for the Company domain and Company name |
| `demo.finished`         | [example](#example-demo-finished)       | [Demo](/apis/demos), [DemoRun](/apis/demos#schema-demo-run), [User Profile](/apis/profiles)                                               | Sent when the Demo is finished; Either the last step is reached **OR** when approximately 30 minutes has passed and the user likely bounced. The `demo_run` will have `finished_kind` of either `"last_step"` or `"timeout_30m"`.                 |
| `demo.form.submitted`   | [example](#example-demo-form-submitted) | [Demo](/apis/demos), [DemoRun](/apis/demos#schema-demo-run), [User Profile](/apis/profiles)                                               | Sent when a Demo form is submitted. The `action` will have the `submission` as a [DemoSubmission](/apis/demos#schema-demo-submission) object. The final demo.finished webhook will contain all of the `actions` and `submissions`            |
...
| `sent_at` | timestamp | The current server time when this webhook was sent (used in [verification](#verification)) |
| `kind` | enum | The [topic identifier](#topics) |
...
**Have an example from your production app to add? Submit a [PR to this file](https://github.com/chamaeleonidae/api/blob/main/mintlify-docs/webhooks/outgoing.mdx) and we'll give you $25 Amazon credit via our Docs Bounty program!**
...
**`question_id` is the discriminator.** It is present only on a Question's answer Button. Branch on it rather than on `tour_action`, which an answer Button does not reliably carry — a subscriber switching on that field sees `undefined` for a Question answer.

| Shape           | Identified by            | Properties to expect                                                                                                                       |
|-----------------|--------------------------|--------------------------------------------------------------------------------------------------------------------------------------------|
| Page Button     | no `question_id`         | `tour_action`, `position`, `additional_actions` and the `style_*` properties — the full [Button](/apis/buttons) schema                      |
| Question Button | `question_id` is present | `question_id`, `order`, `enabled`, plus `score_type` on an `nps` Question. `tour_action` and `position` are normally absent — see below     |
...
| `enabled`     | boolean | Whether this Button is offered. Only meaningful for a `multibutton` Question — a `rating` and an `nps` always present their full preset set                                     |
...
> For `survey.button.clicked` the `button` object has [two shapes](#schema-button-survey-button-clicked) — a Question's answer Button carries `question_id` and no `tour_action`.
