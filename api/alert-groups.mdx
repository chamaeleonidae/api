...
In the HTTP Response for [Listing](/api/alert-groups.md?id=alert-groups-index) and [Showing](/api/alert-groups.md?id=alert-groups-show) Alert Groups,
...
| `kind` | string | The kind of matched Experiences for this Alert Group. [More info ↓](/api/alert-groups.md?id=alert-group-kinds): One of `all`, `tour`, `survey`, `tags`, or `campaigns` |
| `interval` | number | # of days that the alerting condition should wait before triggering a violation |
| `published_at` | timestamp | The time this was most recently published |
| `style` | string | The style of alert condition to evaluate. [More info ↓](/api/alert-groups.md?id=alert-group-styles): One of `unseen` or `uncompleted` |
| `last_notified_at` | timestamp | Last time this alert was triggered for any Experience |
| `tag_ids` | array | When `kind=tags`, the IDs of the [Tags](/api/tags) that Experiences must be tagged with to be included in this Alert Group |
