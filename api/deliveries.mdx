...
| `profile_id` | ID | The Chameleon ID of the [User Profile](/api/profiles.md?id=schema) |
| `idempotency_key` | string | A key that is used to enforce server-side "at most once delivery" for the given `profile_id`. [Learn more ↓](/api/deliveries.md?id=idempotency) |
...
first [list the Deliveries by User Profile](/api/deliveries.md?id=deliveries-index) to determine which ones to remove.
...
**Managing Delivery lifecycle**: When delivering for multiple use cases or when using different versions of `from` and `until` you may need to implicitly manage the "current set of deliveries for a user". This can be done **_directly_** with the API for [Removing a Delivery ↓](/api/deliveries.md?id=deliveries-destroy)
...
| *others | optional | Any other params from [Create a Delivery](/api/deliveries.md?id=deliveries-create)                         |
