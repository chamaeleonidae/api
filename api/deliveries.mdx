...
<Warning>An Experience that is delivered to the User will show immediately and with higher priority than any other Automatic Experiences but an Experience _may not_ show due to the conditions added to the Delivery itself. For example, when a Delivery uses segmentation (`use_segmentation`) and the segmentation does not match the User at the time of the delivery, the Experience will not show and the Delivery is not attempted again.</Warning>

## Delivery Operations

<CardGroup cols={2}>
  <Card title="List Deliveries" icon="list" href="#list-all-deliveries">
    Get all deliveries with filtering options
  </Card>
  <Card title="Create Delivery" icon="plus" href="#create-a-delivery">
    Trigger experiences for specific users
  </Card>
  <Card title="Update Delivery" icon="pen" href="#update-a-delivery">
    Modify delivery timing and parameters
  </Card>
  <Card title="Remove Delivery" icon="trash" href="#remove-a-delivery">
    Cancel or delete scheduled deliveries
  </Card>
</CardGroup>
...
| `profile_id` | ID | The Chameleon ID of the [User Profile](/api/profiles#schema) |
| `idempotency_key` | string | A key that is used to enforce server-side "at most once delivery" for the given `profile_id`. [Learn more ↓](/api/deliveries#idempotency) |
...
| `interaction_id` | ID | The Chameleon ID of [Interaction](/api/tour-interactions) this Delivery triggered. |
...
first [list the Deliveries by User Profile](/api/deliveries#deliveries-index) to determine which ones to remove.
...
**Managing Delivery lifecycle**: When delivering for multiple use cases or when using different versions of `from` and `until` you may need to implicitly manage the "current set of deliveries for a user". This can be done **_directly_** with the API for [Removing a Delivery ↓](/api/deliveries#deliveries-destroy)
...
Mirrors to the options for [Showing an Experience via JavaScript](/js/show-tour#options)
...
| `until`            | optional | string | The [time interval](/concepts/normalization#timestamps) after which this Delivery is no longer valid (i.e. `"+30d"` => 30 days from now, `"+62d"` => 62 days from now) |
...
<Info>Required: One of `profile_id`, `uid` or `email`</Info>
...
| *others | optional | Any other params from [Create a Delivery](/api/deliveries#deliveries-create)                         |
