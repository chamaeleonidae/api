...
See the specific APIs for more information on how to use Search your product.
 ...
 - [REST API](/api/helpbar#helpbar-rest-api)
 - [JavaScript API](/api/helpbar#search-javascript-api)
 - [Limits](/api/helpbar#limits)


#### To get HelpBar up and running, complete these items:

1. Create a [HelpBar](/api/helpbar#search-bar-schema), configure it with `placeholder` etc.
2. Add your Help center as a [`SearchGroup`](/api/helpbar#searchgroups-schema).
3. Add the top-level navigation within your product; First download this CSV [template](/api/helpbar#search-imports-template) or [example](/api/helpbar#search-imports-example) then [Import](/api/helpbar#schema-search-imports) it.
4. [Optional] Add specific content from your database by creating a [SearchItem](/api/helpbar#schema-search-items) per database record you want to be searchable.
...
<CardGroup cols={2}>
  <Card title="Index Custom Content" icon="database" href="#searchitem-schema">
    Index custom content from your product into searchable items
  </Card>
  <Card title="Import Navigation" icon="map" href="#schema-search-imports">
    Import your product navigation for direct user jumping
  </Card>
  <Card title="Configure Groups" icon="folder" href="#searchgroups-schema">
    Pull content from public sources like help centers and blogs
  </Card>
  <Card title="Create Labels" icon="tag" href="#searchlabel-schema">
    Add labels to search items for better visibility and organization
  </Card>
  <Card title="Manage HelpBar" icon="bars" href="#list-all-helpbars">
    Create and configure HelpBar instances
  </Card>
  <Card title="Label Themes" icon="palette" href="#searchlabeltheme-schema">
    Define custom styling for search item labels
  </Card>
</CardGroup>
...
`Search` is associated with `groups`: [SearchGroups](/api/helpbar#schema-search-groups), and `label_themes`: [SearchLabelThemes](/api/helpbar#schema-search-groups)
...
| `url_group_ids`               | array     | The [`Environment`](/api/urls#url-groups) that this is enabled to run on                                                                                    |
...
| `oids`                        | none      | A set of pre-known IDs to directly add [SearchItem](/api/helpbar#schema-search-items)s to the default [SearchGroup](/api/helpbar#schema-search-groups)s |
...
| `created_at`                | timestamp | When this happened or when this was added to the Database                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `updated_at`                | timestamp | The last time any property was updated                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `url`                       | string    | The URL that this [SearchGroup](/api/helpbar#schema-search-groups) uses to fetch Search content                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `icon`                      | object    | The `SearchIcon` info to display for all results of this [SearchGroup](/api/helpbar#schema-search-groups)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `icon.image_url`            | none      | When using `kind="image"`, the url to an appropriate icon for this [SearchGroup](/api/helpbar#schema-search-groups)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `icon.image_alt`            | none      | When using `kind="image"`, the alt text for the icon                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `url_options`               | object    | When configuring this [SearchGroup](/api/helpbar#schema-search-groups) for an Search integration, Chameleon will fetch some information about the given URL                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
There is only one per account so an update uses the same endpoint as create. Use any of the properties available in the [schema](/api/helpbar#schema)
...
> `SearchItem`s are associated with `actions`: [SearchActions](/api/helpbar#schema-search-actions), and `title_labels` / `description_labels`: [SearchLabels](/api/helpbar#schema-search-labels)
...
| `search_group_id`    | string    | The [SearchGroup](/api/helpbar#schema-search-groups) that this `SearchItem` will display in.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `icon.image_url`     | sting     | When using `kind="image"`, the url to an appropriate icon for this [SearchGroup](/api/helpbar#schema-search-groups)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `icon.image_alt`     | string    | When using `kind="image"`, the alt text for the icon                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `actions`            | array     | An array of the [`SearchAction`](/api/helpbar#schema-search-actions) that run when this `SearchItem` is clicked/selected in the HelpBar                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `title_labels`       | array     | An array of the [`SearchLabel`](/api/helpbar#schema-search-labels) items displayed in the title of this `SearchItem`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `description_labels` | array     | An array of the [`SearchLabel`](/api/helpbar#schema-search-labels) items displayed under the description of this `SearchItem`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `search_import_id` | optional | Fetch only the `SearchItem`s that were created (or updated most recently) by the given [SearchImport](/api/helpbar#schema-search-imports). |
...
Use any of the properties available in the [schema](/api/helpbar#schema-search-items).
...
Chameleon [Companies](/api/companies#companies-show) and then send `company_ids` parameter.
...
It also includes a title label of `NEW` themed with an [example theme](/api/helpbar#searches-create-http-response) from above.
...
| `*others` | optional | Use any of the properties available in the [single SearchItem update](/api/helpbar#search-items-create) |
...
To update the [SearchGroup](/api/helpbar#schema-search-group) of many items:
...
A search action is one item in a collection of actions attached to a [SearchItem](/api/helpbar#schema-search-items)s. When
...
[Merge tags](/concepts/personalization) are available for the `href` field; add the right Calendly link as [User data](/webhooks/profiles) and merge it in the action. (`csm` below refers to a customer success manager)
...
See the [Type definitions below ↓](/api/helpbar#search-js-types)
...
| `search_group_id`           | string    | The [SearchGroup](/api/helpbar#schema-search-groups) that all of the resulting `SearchItem`s will display in.  (and/or this can be customized per row with "HelpBar Group ID" column) |
...
| `stats`                     | object    | The details of the data itself and of the last run of this [SearchImport](/api/helpbar#schema-search-imports).                                                      |
