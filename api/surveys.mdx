...
<CardGroup cols={2}>
  <Card title="List Microsurveys" icon="list" href="#list-microsurveys">
    Get all microsurveys with filtering and pagination
  </Card>
  <Card title="Retrieve Microsurvey" icon="eye" href="#retrieve-a-microsurvey">
    Get a specific microsurvey by ID
  </Card>
  <Card title="Update Microsurvey" icon="pen" href="#update-a-microsurvey">
    Modify existing microsurvey properties
  </Card>
  <Card title="Filter by Segment" icon="filter" href="#filtering-by-segment">
    Find microsurveys targeted to specific user segments
  </Card>
</CardGroup>
...
| `segment_id` | ID | The Chameleon ID of the configured [Segment](/api/segments#schema) |
| `published_at` | timestamp | The time this was most recently published |
| `tag_ids` | array&lt;ID&gt; | The Chameleon IDs of the [Tags](/api/tags) attached to this model |
...
Update a single Microsurvey to change the [Environments](/api/urls) or to Publish it.
...
| `urls_group_id` | optional | An [Environments](/api/urls) ID prefixed with `+` to add or or `-` to remove the Environment |
...
See [Listing Related Models](/api/segments#segment-experiences-index).
