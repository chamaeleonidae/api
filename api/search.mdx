...
- Index custom content (the data from your product) into searchable [SearchItem](/api/search#schema-search-items)s.
- Import your product navigation to allow your Users to jump to a specific thing they are looking for with a [SearchImport](/api/search#schema-search-imports).
- Configure [SearchGroups](/api/search#schema-search-groups) to pull content from public sources (i.e. your Help center, Blog etc.).
- Define a [SearchLabelTheme](/api/search#schema-search-label-themes) to label your [SearchItem](/api/search#schema-search-items)s for greater visibility.
- Add labels to the title or description of `SearchItem`s as [SearchLabel](/api/search#schema-search-labels)s.
