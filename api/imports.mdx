...
You may Import data into a [User Profiles](/api/profiles) or a [Company](/api/companies) with the `model_kind` property.
...
| `on_model_missing`          | string                     | The strategy to use when data present in the Import is missing in Chameleon (i.e. [User Profile](/api/profiles) or [Company](/api/companies) has **not yet** been identified to Chameleon): One of `create` or `ignore`                                                                                         |
...
| `properties`                | array&lt;Property&gt;      | The list of definitions of how to map CSV column headers to [Properties](/api/properties) on the model. [example ↓](/api/imports#examples-profiles-tag-all)                                                                                                                                                                               |
| `properties.$.name`         | string                     | The column header of this property in the CSV file                                                                                                                                                                                                                                         |
| `properties.$.prop`         | string                     | The `prop` value of the [Property](/api/properties) to store on the model. New properties are created dynamically for missing `prop` values.                                                                                                                                                           |
...
- Check out the [cURL Examples](/api/imports#examples-all-curl) below to see this in action.
...
| `properties`        | required | array&lt;Property&gt; | The list of definitions of how to map CSV column headers to [Properties](/api/properties) on the model. |
| `properties.$.name` | required | string                | The column header of this property in the CSV file |
| `properties.$.prop` | required | string                | The `prop` value of the [Property](/api/properties) to store on the model |
...
<Info>For CSV-based import `kind`, both valid `properties` and `file` are required before `import_at` can be set.</Info>
...
| `409` | The number of Imports limit has been reached, see [limits ↑](/api/imports#limits) for details                    |
...
| `422` | The file is larger than the current limits [limits ↑](/api/imports#limits) allow                                 |
| `422` | The file has more columns than the current limits [limits ↑](/api/imports#limits) allow                          |
...
<Warning>You must set `on_model_missing` to `ignore` because no field maps to the `uid` Property.</Warning>
...
With a CSV `file` like this, use `properties` to specify the headers as mapped to the `prop` field of Chameleon [Property](/api/properties).
...
**See options for [Creating an Import](/api/imports#imports-create)**
...
Same endpoint as [show](/api/imports#imports-show), but adds a `.csv` on the end. The response body is the CSV data.
