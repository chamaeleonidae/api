...
<CardGroup cols={2}>
  <Card title="List Launchers" icon="list" href="#list-launchers">
    Get all launchers with filtering parameters
  </Card>
  <Card title="Retrieve Launcher" icon="eye" href="#retrieve-a-launcher">
    Get a specific launcher by ID
  </Card>
  <Card title="Update Launcher" icon="pen" href="#update-a-launcher">
    Modify existing launcher properties
  </Card>
  <Card title="Filter by Segment" icon="filter" href="#filtering-by-segment">
    Find launchers for specific user segments
  </Card>
</CardGroup>
...
| `campaign_id` | ID | The Chameleon ID of the [Tour](/api/tours) or [Microsurvey](/api/surveys) referenced when `kind=tour` or `kind=survey` |
